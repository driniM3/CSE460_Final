...
namespace ProjectManagement.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(string tenant)
        {
            ...
            {
                @ViewBag.tenant = db.Tenants.Where(x => x.Name.ToLower() == tenant.ToLower()).Single().Name;
                return View();
            }
            else if (tenant == "New")
            {
        ...
        [AllowAnonymous]
        public ActionResult Register(string tenant)
        {
            var db = new PMEntities();
            if(db.Tenants.Any(x=> x.Name.ToLower() == tenant.ToLower())){
                @ViewBag.tenant = db.Tenants.Where(x=> x.Name.ToLower() == tenant.ToLower()).Single().Name;
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    if (model.Tenant == "New")
                    {
                        ...
                        System.Web.Security.Roles.AddUserToRole(model.UserName, "Tenant Manager");
                        var tenant = new Tenant
                        {
                            Name = "Tenant" + (db.Tenants.Count() + 1),
